...
if ((isset($_GET['aksi']) && $_GET['aksi'] == 'simpan'))
	...
	if ((empty($euser)) OR (empty($epass)))
		...
		{
		$filenamex = "filebox/user/$euser";
		$filenamexnya = "filebox/user/$euser/$euser.txt";
		...
		if (!file_exists($filenamex)) 
			...
			{
			//betul.... re-direct			
		    //echo "betul [$filenamex]";
		    
		    //dapatkan kd
		    $myfile = fopen($filenamexnya, "r") or die("Unable to open file!");
			$kdnya = fread($myfile,filesize($filenamexnya));
			fclose($myfile);
			...
			//cek
			$qku = mysql_query("SELECT * FROM $yuk1x ".
